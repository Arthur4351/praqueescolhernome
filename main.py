...
# =========================================================================
# INTERCEPTADOR DE LINHA DE COMANDO
#   Caso 1 (-m py_compile arquivo.py): apenas valida sintaxe (inofensivo).
#   Caso 2 (arquivo.py): DESATIVADO — não executa scripts arbitrários (RCE).
# =========================================================================
if len(sys.argv) > 1:
    ...
    # Caso 2: Execução de script dinâmico — DESATIVADO POR SEGURANÇA.
    # Executar um arquivo .py arbitrário passado por linha de comando era um
    # vetor de execução de código arbitrário (RCE). O recurso fica recusado:
    # nada é lido nem executado. (A geração de código por IA está estacionada.)
    elif sys.argv[1].endswith(".py") and os.path.exists(sys.argv[1]):
        sys.stderr.write("Execução de scripts dinâmicos desativada por segurança.\n")
        sys.exit(2)
...
class SophiaApp(QMainWindow):
    ...
    def _auto_execute_pending(self):
        ...
        if intent in ("EDIT_PROJECT_FILE", "GENERATE_NEW_SKILL"):
            self.log_callback(
                "🔒 <b>SOPHIA:</b> Geração e edição automática de código está "
                "<b>desativada por segurança</b> nesta versão. Esse recurso de IA "
                "está em desenvolvimento e será reativado em uma etapa futura."
            )
            self.ai.pending_intents = []
            self.estado_atual = AppStatus.LIVRE
            return
    ...
    def _execute_auditoria_flow(self):
        ...
        if not excel_path:
            ...
            return
        def run_audit():
            # log_func precisa emitir via signal: auditar_efetivo roda na thread do
            # Worker e NAO pode tocar o chat_display direto (regra de thread do Qt).
            self.executor.auditar_efetivo(
                d_pasta, excel_path,
    ...
    def enviar_comando(self):
        ...
        self.chat_display.append(f"<div><b>Você:</b> {html.escape(txt_original)}</div>")
        ...
        if txt == "scan":
            ...
            if not self.executor:
                ...
                worker = Worker(
                    self.executor.atualizar_datas_planilha,
                    e, f,
                    lambda msg: worker.signals.log_signal.emit(msg)
                )
                worker.signals.log_signal.connect(self.log_callback)
    ...
    def _execute_final_visual_step(self, intent: str):
        ...
        try:
            ...
            if intent == "CREATE_FOLDER":
                ...
                worker = Worker(
                    self.executor.atualizar_datas_planilha,
                    self.ai.args["excel"], self.ai.args["fotos"],
                    lambda msg: worker.signals.log_signal.emit(msg),
                    dynamic_filter
                )
                worker.signals.log_signal.connect(self.log_callback)
                ...
                if not hasattr(self.executor, "replicar_formatacao_colunas"):
                    self.log_callback(
                        "🔒 <b>SOPHIA:</b> Replicação de formatação de colunas ainda "
                        "não está implementada nesta versão. Recurso em desenvolvimento."
                    )
                    self.ai.pending_intents = []
                    self.estado_atual = AppStatus.LIVRE
                    return
                ...
                args_repl = self.ai.args.copy()
                worker = Worker(
                    self.executor.replicar_formatacao_colunas,
                    args_repl["arquivo_origem"],
                    args_repl["aba_origem"],
                    args_repl["pasta_destino"],
                    lambda msg: worker.signals.log_signal.emit(msg)
                )
                worker.signals.log_signal.connect(self.log_callback)
                worker.signals.finished_signal.connect(self.log_callback)
