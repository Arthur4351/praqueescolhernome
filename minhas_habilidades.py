...
class SophiaExecutor:
            ...
            for nome_aba in wb.sheetnames:
                ...
                for r in range(1, 6):
                    for c in range(1, ws.max_column + 1):
                        ...
                        # Palavra inteira: evita casar 'DIA' dentro de 'MÉDIA' (média).
                        if v and re.search(r'\b(DATA|DIA)\b', str(v).upper()):
                            coluna_data = c
                            row_cabecalho = r
                            break
    ...
    def _descobrir_dia_para_celula(self, ws, r, c, max_c, dia_fallback=None):
        """Descobre a qual DIA pertence a celula (r, c).

        Suporta dois layouts:
        - Vertical (dias um embaixo do outro): retorna o ultimo DIA visto acima.
        - Lado a lado (ex: DIA 1 ao lado do DIA 2, DIA 3/4 na linha de baixo):
          procura o rotulo DATA/DIA mais proximo a esquerda na mesma linha
          ou nas linhas acima, garantindo que o bloco da esquerda nao roube
          o dia do bloco da direita (e vice-versa).

        Retorna o dia como string ou dia_fallback se nada for encontrado.
        """
        MAX_LINHAS_ACIMA = 30
        ALCANCE_ESQUERDA = 30
        try:
            r_min = max(1, r - MAX_LINHAS_ACIMA)
            for rr in range(r, r_min - 1, -1):
                c_min = max(1, c - ALCANCE_ESQUERDA)
                for cc in range(c, c_min - 1, -1):
                    v = ws.cell(row=rr, column=cc).value
                    if not v:
                        continue
                    v_norm = remover_acentos(v).replace(" ", "").replace(".", "")
                    if not (v_norm.startswith('DATA') or v_norm.startswith('DIA')):
                        continue
                    # O rotulo precisa estar na mesma coluna ou a esquerda
                    # da celula (nunca a direita) para nao cruzar blocos.
                    if cc > c:
                        continue
                    for off in range(0, 7):
                        col_dia = cc + off
                        if col_dia >= max_c or col_dia < 1:
                            break
                        v_off = ws.cell(row=rr, column=col_dia).value
                        if not v_off:
                            continue
                        # Evita confundir "FOTO01"/"SERVICO02" com numero do dia.
                        v_off_norm = remover_acentos(v_off).replace(" ", "").replace(".", "")
                        if off > 0 and any(k in v_off_norm for k in [
                            'FOTO', 'ENTRADA', 'SAIDA', 'ANTES', 'DEPOIS',
                            'SERVICO', 'ATIVIDADEREALISADA', 'ATIVIDADEREALIZADA'
                        ]):
                            continue
    ...
    def _injetar_aba(self, ws, aba_norm, dias_cache, dados_relatorio):
        ...
        for r in range(1, max_r):
            ...
            for c in range(1, max_c):
                ...
                # Prefixo (não substring): 'MEDIA' não deve casar com 'DIA'.
                if v_norm.startswith('DATA') or v_norm.startswith('DIA'):
                ...
                # Descobre o dia DONO desta celula (por bloco/coluna).
                # Sem isso, num layout lado-a-lado (1|2 em cima, 3|4 embaixo)
                # o dia_atual global faria os dois blocos da mesma linha
                # herdarem o ultimo dia (2 e 4), perdendo os dias 1 e 3.
                dia_da_celula = self._descobrir_dia_para_celula(ws, r, c, max_c, dia_atual)
                if dia_da_celula:
                    dias_na_planilha.add(dia_da_celula)

                if dia_da_celula and dia_da_celula in dias_cache:
                    fots = dias_cache[dia_da_celula]
    ...
    def _aba_casa_com_pasta(self, aba_norm, eq_key):
        """Diz se a aba do Excel corresponde a pasta de fotos.

        Exato tem prioridade. O fuzzy antigo (ratio > 0.75) fazia
        'HIDROJATO 01' casar com 'HIDROJATO 02' (só muda 1 dígito,
        ratio ~0.9) e espalhava as fotos do 01 em todas as abas.
        Por isso: se ambos têm números e os números diferem, NUNCA casa.
        """
        if not aba_norm or not eq_key:
            return False
        if aba_norm == eq_key:
            return True
        try:
            nums_aba = [int(n) for n in re.findall(r'\d+', aba_norm)]
            nums_eq = [int(n) for n in re.findall(r'\d+', eq_key)]
        except (ValueError, TypeError):
            nums_aba, nums_eq = [], []
        if nums_aba and nums_eq and nums_aba != nums_eq:
            return False
        if aba_norm in eq_key or eq_key in aba_norm:
            return True
        try:
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if excel_lock_callback:
                        ...
                        _tentativas_lock += 1
                        if _tentativas_lock >= _MAX_TENTATIVAS_LOCK:
                            log_func(f"❌ O Excel ('{Path(excel_path).name}') continua bloqueado após {_MAX_TENTATIVAS_LOCK} tentativas. Feche o arquivo e tente de novo.")
                            return "Abortado: Excel bloqueado (limite de tentativas atingido)."
                        log_func(f"⚠️ O Excel original ('{Path(excel_path).name}') está aberto! Tentativa {_tentativas_lock}/{_MAX_TENTATIVAS_LOCK}. Nova tentativa em 5 segundos...")
                        import time
                        time.sleep(5)
            ...
            for nm_aba in wb.sheetnames:
                ...
                if pasta_alvo is None:
                    for eq_key, eq_pasta in indice_pastas.items():
                        if self._aba_casa_com_pasta(aba_norm, eq_key):
                            pasta_alvo = eq_pasta
                            break
            ...
            if fotos_total_geral > 0:
                try:
                    ...
                    nome_saida = destino_path / f"Relatorio_{carimbo}_{uuid.uuid4().hex[:6]}.xlsm"
                    wb.save(nome_saida)
    ...
    def auditar_efetivo(self, pasta_efetivo: str, excel_path: str, log_func):
        try:
            pastas_fisicas = [FileHandler.normalize_string(p.name) for p in Path(pasta_efetivo).iterdir() if p.is_dir()]
            
            wb = load_workbook(excel_path, read_only=True, data_only=True)
            ws = wb.active
            # Preserva as POSIÇÕES reais das colunas (não compacta células vazias),
            # senão o índice do cabeçalho não bate com row[col_idx] na leitura das linhas.
            raw_headers = [cell.value for cell in ws[1]]

            colunas_possiveis = [
                i for i, c in enumerate(raw_headers)
                if c is not None and any(k in str(c).lower() for k in ['nome', 'funcionario', 'equipe', 'colaborador'])
            ]
