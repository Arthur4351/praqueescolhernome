...
class AIRouter:
    ...
    _DEFAULT_PROVIDERS = [
        # --- Fase 2: OpenCode Zen (mesma conta/chave OPENCODE_API_KEY) ---
        # NOTA IMPORTANTE (verificado ao vivo): os modelos "-free" da Zen
        # (mimo-v2.6-flash-free, big-pickle, etc.) retornam 403 FreeTierError
        # "can only be used from within OpenCode" quando chamados de um app
        # externo como a Sophia — é uma trava do lado do provedor, o header
        # User-Agent: opencode NÃO contorna mais. A MESMA chave funciona nos
        # modelos pagos/normais (testados 200 OK: qwen3.8-flash, deepseek-v4.1-flash).
        # Por isso o modelo ATIVO abaixo é qwen3.8-flash (rápido e barato). Para
        # usar o MiMo free, seria necessário rodar via CLI do OpenCode (não se
        # aplica aqui). Troque "modelo" abaixo para outro id da Zen se preferir.
        {
            "nome": "OpenCode-Zen",
            "base_url": "https://opencode.ai/zen/v1/chat/completions",
            "env_key": "OPENCODE_API_KEY",
            "modelo": "qwen3.8-flash",
            "modelo_codigo": "qwen3.8-flash",
            "modelo_reasoning": "deepseek-v4.1-flash",
            "modelo_intent": "qwen3.8-flash",
    ...
    def _save_usage(self):
        with self.lock:
            if not FileHandler.save_json_atomic(
                self.usage_file, {"data": self.today, "uso": self.usage_today}, indent=2
            ):
                FileHandler.log_error("AIRouter._save_usage falhou (escrita atômica)")
    ...
    def _call_provider(
        ...
        if "opencode.ai" in provider["base_url"]:
            # Modelos free da OpenCode Zen exigem este User-Agent.
