...
class SophiaAgentCore:
    ...
    def execute_pending_intents(self, global_args: dict = None) -> str:
        ...
        try:
            for item in self.pending_intents:
                ...
                if intent == "CREATE_FOLDER":
                    ...
                    if sucesso:
                        ...
                        resultados.append(f"✅ Pasta criada: {new_path}")
                    else:
                    ...
                    alvo_rn = args.get("target_path", ".")
                    novo_rn = args.get("new_name", "NovoNome")
                    if FileHandler.rename_folder(alvo_rn, novo_rn):
                        resultados.append(f"✅ Pasta renomeada para: {novo_rn}")
                    else:
                        resultados.append(f"⚠️ Não consegui renomear (inexistente?): {alvo_rn}")

                elif intent == "DELETE_FOLDER":
                    alvo_del = args.get("target_path", ".")
                    if FileHandler.delete_folder(alvo_del):
                        resultados.append(f"✅ Pasta apagada: {alvo_del}")
                    else:
                        resultados.append(f"⚠️ Pasta NÃO apagada (recusada por segurança ou inexistente): {alvo_del}")
                    ...
                    destino_mv = FileHandler.move_folder(src, dest)
                    if destino_mv:
                        resultados.append(f"✅ Movido para: {destino_mv}")
                    else:
                        resultados.append(f"⚠️ Não consegui mover: {src}")

                elif intent == "INJECT_FORMULA":
                    cell = args.get("cell") or args.get("cell_coord")
                    if ExcelEngine.inject_formula(args.get("excel_path"), args.get("sheet_name"), cell, args.get("formula")):
                        resultados.append(f"✅ Fórmula inserida em {cell}.")
                    else:
                        resultados.append(f"⚠️ Não consegui inserir a fórmula em {cell}.")

                elif intent in ("EDIT_PROJECT_FILE", "GENERATE_NEW_SKILL"):
                    # Geração/edição automática de código DESATIVADA por segurança.
                    # Evita execução de código arbitrário (RCE). Recurso de IA em
                    # desenvolvimento — será reativado em uma etapa futura.
                    resultados.append(
                        f"🔒 Ação [{intent}] ignorada: geração de código está "
                        f"desativada por segurança nesta versão."
                    )
                    continue
                    ...
                    try:
                        ...
                        if not FileHandler.save_json_atomic(self.config_path, self.config, indent=4, ensure_ascii=False):
                            raise Exception("escrita atômica do config.json falhou")
                    ...
                    resultados.append(f"⚠️ Intent não reconhecida nesta versão: [{intent}] (nenhuma ação executada)")
            ...
            houve_aviso = any(
                any(marca in r for marca in ("⚠️", "🔒", "❌")) for r in resultados
            )
            cabecalho = (
                "⚠️ Lote processado (com avisos)"
                if houve_aviso else "✅ Lote executado com sucesso"
            )
            return f"{cabecalho} — memória limpa: " + " | ".join(resultados)
