...
class FileHandler:
    ...
    def delete_folder(target_path: str) -> bool:
        """Remove uma pasta recursivamente COM salvaguardas contra alvos catastróficos.
        Recusa: alvo vazio ou '.', raiz de disco (ex.: C:\\), a pasta pessoal do
        usuário e o diretório de trabalho atual. Exige pasta existente."""
        try:
            if not target_path or not str(target_path).strip() or str(target_path).strip() == ".":
                FileHandler.log_error(f"FileHandler: delete_folder recusado (alvo vazio/'.'): {target_path!r}")
                return False

            target = Path(target_path).resolve()

            # Recusa raiz de disco / raiz do sistema de arquivos (ex.: C:\, D:\, /)
            if target == target.parent:
                FileHandler.log_error(f"FileHandler: delete_folder recusado (raiz de disco): {target}")
                return False

            # Recusa a pasta pessoal do usuário, o diretório de trabalho atual E
            # qualquer ANCESTRAL deles (ex.: ".." resolve para o pai do cwd e
            # apagaria o projeto inteiro).
            protegidos = set()
            for getter in (Path.home, Path.cwd):
                try:
                    protegidos.add(getter().resolve())
                except Exception:
                    pass
            for base in protegidos:
                if target == base or target in base.parents:
                    FileHandler.log_error(f"FileHandler: delete_folder recusado (protegida/ancestral): {target}")
                    return False
    ...
    @staticmethod
    def save_json_atomic(target_path, data, indent: int = 2, ensure_ascii: bool = False) -> bool:
        """Grava JSON de forma ATÔMICA: escreve num arquivo temporário no mesmo
        diretório e faz os.replace (atômico no mesmo volume). Se o processo morrer
        no meio da escrita, o arquivo original permanece intacto — evita corromper
        brain.json, long_term_memory.json, ai_usage.json e config.json."""
        import json
        import os
        import tempfile
        try:
            ...
            FileHandler.log_error(f"FileHandler.save_json_atomic falhou para {target_path} - {e}")
            return False
