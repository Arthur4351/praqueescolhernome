...
class IntentParser:
    ...
    def _save_brain(self):
        """Salva as intenções físicas de volta no JSON de forma silenciosa (escrita atômica)."""
        if self._intents_bow:
            data = {k: list(v) for k, v in self._intents_bow.items()}
            with self.lock:
                if not FileHandler.save_json_atomic(self.brain_file, data, indent=4, ensure_ascii=False):
                    FileHandler.log_error("Erro ao salvar brain.json (escrita atômica falhou)")
            ...
            try:
                ...
                if os.path.exists(memory_file):
                    try:
                        ...
                        # Arquivo corrompido NÃO pode travar a memória para sempre: recomeça do zero.
                        FileHandler.log_error("IntentParser._save_memory: long_term_memory.json corrompido — recriando.")
                        memories = []
                if fato not in memories:
                    memories.append(fato)
                if not FileHandler.save_json_atomic(memory_file, memories, indent=2, ensure_ascii=False):
                    raise Exception("escrita atômica falhou")
    ...
    def _cloud_inference(self, user_input: str, user_name: str = "Usuário"):
        ...
        system_prompt = (
            ...
            f"IMPORTANTE: Você NÃO gera nem executa código, scripts, macros ou VBA, e não cria arquivos do zero. Se pedirem isso, explique com estilo que essa função está desativada e ofereça as ações que você realmente faz (listadas abaixo).\n\n"
            ...
            "INJECT_FORMULA, REGISTRAR_FATO_LTM, UPDATE_WHATSAPP, CONFIGURAR_PADRAO_FOTOS.\n"
            "Regra: você cria PASTAS (CREATE_FOLDER), renomeia/move pastas, injeta fórmulas nativas no Excel (INJECT_FORMULA), consulta planilhas (QUERY_EXCEL/QUERY_COUNT_EMPTY), processa fotos (PROCESSAR_FOTOS) e corrige datas (DATAS). Você NÃO gera scripts, macros/VBA nem cria arquivos do zero — se pedirem, avise com estilo que essa parte está desativada. Para 'crie a pasta X' use CREATE_FOLDER normalmente.\n"
            "Params por tarefa:\n"
            ...
            "- PROCESSAR_FOTOS: Use EXCLUSIVAMENTE quando o usuário pedir para processar, salvar, rodar ou injetar FOTOS/IMAGENS na planilha Excel (ex: 'salva foto com o padrão', 'processe as fotos', 'injetar fotos'). Se não envolver FOTOS/IMAGENS de forma clara, NÃO use PROCESSAR_FOTOS.\n"
            ...
            "2. Si o usuário pedir para PROCESSAR, SALVAR ou INJETAR FOTOS ou IMAGENS (ex: 'salve fotos', 'processar fotos', 'salva foto com o padrão do sistema'), ele quer que você EXECUTE o processamento das imagens, então use PROCESSAR_FOTOS.\n"
    ...
    def _check_and_upgrade_intent(self, res: dict) -> dict:
        """[DESATIVADO] A geração dinâmica de código (GENERATE_NEW_SKILL) está estacionada
        por segurança. Este método antes promovia CREATE_FOLDER → GENERATE_NEW_SKILL quando
        havia palavras de arquivo (planilha/excel/etc). Agora é um no-op: CREATE_FOLDER
        continua sendo CREATE_FOLDER (ação funcional e determinística), e nada é roteado
        para o intent estacionado. Mantido como ponto de extensão para a Fase 2.
        """
        return res
