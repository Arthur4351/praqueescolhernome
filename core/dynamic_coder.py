...
class DynamicCoder:
    ...
    def edit_project_file(self, target_file_path: str, instruction: str) -> str:
        """[DESATIVADO POR SEGURANÇA] Antes reescrevia arquivos REAIS do projeto com
        código gerado por LLM (backup .bak + write_text + py_compile). Essa capacidade
        foi removida — retorna recusa imediata. Os intents EDIT_PROJECT_FILE/
        GENERATE_NEW_SKILL também estão estacionados na camada de execução
        (core/agent_core.py, main.py). Mantido só para compatibilidade de import."""
        return (
            "🔒 Edição automática de arquivos do projeto está desativada por "
            "segurança nesta versão."
        )
        # --- código abaixo mantido inerte (inalcançável) ---
        if not self.api_key:
        ...
        # [DESATIVADO POR SEGURANÇA] escrita de código gerado por LLM em arquivo
        # real do projeto removida — nada é gravado no disco.
        _ = new_code  # inerte
    ...
    def generate_and_run(self, task_prompt: str, context_args: dict = None) -> str:
        """[DESATIVADO POR SEGURANÇA] Antes gerava um script via LLM e o EXECUTAVA
        via subprocess (RCE de código gerado por IA). A execução foi removida —
        retorna recusa imediata. Mantido só para compatibilidade de import."""
        return (
            "🔒 Geração e execução de scripts dinâmicos está desativada por "
            "segurança nesta versão."
        )
        # --- código abaixo mantido inerte (inalcançável) ---
        if not self.api_key:
        ...
        try:
            ...
            # 3. [DESATIVADO POR SEGURANÇA] Execução de scripts gerados por LLM
            # removida (era RCE). Nenhum subprocess é iniciado.
            class _ResultadoInerte:
                returncode = 126
                stdout = ""
                stderr = "Execução de scripts dinâmicos desativada por segurança."
            result = _ResultadoInerte()
